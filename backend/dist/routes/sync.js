...
router.get('/events', async (req, res) => {
    ...
    let filteredEvents = (0, eventBus_1.getEventsSince)({ since, lastEventId, limit });
    ...
    if (board_id) {
        ...
        if (columnIds.size > 0) {
            ...
            const columns = await new Promise((resolve, reject) => {
                database_1.db.all(`SELECT id, board_id FROM columns WHERE id IN (${placeholders})`, columnIdArray, (err, rows) => {
                    if (err)
                        reject(err);
                    else
                        resolve(rows || []);
    ...
    if (priority) {
        filteredEvents = filteredEvents.filter(event => event.data && event.data.task && event.data.task.priority === priority);
    }
...
router.get('/stream', (req, res) => {
    ...
    const backlog = (0, eventBus_1.getEventsSince)({
        since,
        lastEventId: headerLastEventId || lastEventId,
        limit,
    });
    backlog.forEach((event) => writeEvent(res, event));
    const unsubscribe = (0, eventBus_1.subscribe)((event) => {
