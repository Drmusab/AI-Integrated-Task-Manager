...
router.get('/priorities', [
    (0, express_validator_1.query)('date').optional().isISO8601().toDate(),
], async (req, res) => {
    try {
        const date = req.query.date || new Date().toISOString().split('T')[0];
        const priorities = await (0, database_1.allAsync)(`SELECT dp.*, t.title as task_title, t.column_id, t.priority as task_priority
       FROM daily_priorities dp
       LEFT JOIN tasks t ON dp.task_id = t.id
       WHERE dp.date = ?
       ORDER BY dp.position ASC`, [date]);
...
router.post('/priorities', [
    (0, express_validator_1.body)('date').notEmpty().isISO8601(),
    (0, express_validator_1.body)('position').isInt({ min: 0, max: 2 }).withMessage('Position must be 0, 1, or 2'),
    (0, express_validator_1.body)('title').notEmpty().withMessage('Title is required'),
    (0, express_validator_1.body)('completed').optional().isBoolean(),
    (0, express_validator_1.body)('taskId').optional().isInt(),
], async (req, res) => {
    ...
    try {
        ...
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO daily_priorities (date, position, title, completed, task_id, updated_at)
       VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [date, position, title, completed ? 1 : 0, taskId]);
...
router.put('/priorities/:id', [
    ...
    (0, express_validator_1.body)('completed').optional().isBoolean(),
    (0, express_validator_1.body)('taskId').optional(),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        const existing = await (0, database_1.getAsync)('SELECT * FROM daily_priorities WHERE id = ?', [priorityId]);
        if (!existing) {
            return res.status(404).json({ error: 'Priority not found' });
        }
        await (0, database_1.runAsync)(`UPDATE daily_priorities SET
        title = COALESCE(?, title),
        completed = COALESCE(?, completed),
        task_id = COALESCE(?, task_id),
        updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`, [title, completed !== undefined ? (completed ? 1 : 0) : null, taskId, priorityId]);
 ...
 * Delete a priority
 */
router.delete('/priorities/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const result = await (0, database_1.runAsync)('DELETE FROM daily_priorities WHERE id = ?', [req.params.id]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Priority not found' });
        }
...
router.get('/notes', [
    ...
    try {
        const date = req.query.date || new Date().toISOString().split('T')[0];
        const note = await (0, database_1.getAsync)('SELECT * FROM daily_notes WHERE date = ?', [date]);
        res.json(note ? {
            id: note.id,
            date: note.date,
...
router.post('/notes', [
    ...
    try {
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO daily_notes (date, content, updated_at)
       VALUES (?, ?, CURRENT_TIMESTAMP)`, [date, content]);
...
router.get('/reflections', [
    ...
    try {
        const date = req.query.date || new Date().toISOString().split('T')[0];
        const reflection = await (0, database_1.getAsync)('SELECT * FROM daily_reflections WHERE date = ?', [date]);
        res.json(reflection ? {
            id: reflection.id,
...
router.post('/reflections', [
    (0, express_validator_1.body)('date').notEmpty().isISO8601(),
    (0, express_validator_1.body)('wentWell').optional().isString(),
    (0, express_validator_1.body)('couldImprove').optional().isString(),
    (0, express_validator_1.body)('keyTakeaways').optional().isString(),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO daily_reflections (date, went_well, could_improve, key_takeaways, updated_at)
       VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)`, [date, wentWell, couldImprove, keyTakeaways]);
...
router.get('/time-blocks', [
    ...
    try {
        const date = req.query.date || new Date().toISOString().split('T')[0];
        const blocks = await (0, database_1.allAsync)(`SELECT tb.*, t.title as task_title, t.priority as task_priority
       FROM time_blocks tb
       LEFT JOIN tasks t ON tb.task_id = t.id
       WHERE tb.date = ?
       ORDER BY tb.start_time ASC`, [date]);
...
router.post('/time-blocks', [
    (0, express_validator_1.body)('date').notEmpty().isISO8601(),
    (0, express_validator_1.body)('startTime').notEmpty().matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/),
    (0, express_validator_1.body)('endTime').notEmpty().matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/),
    (0, express_validator_1.body)('title').notEmpty().withMessage('Title is required'),
    ...
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO time_blocks (date, start_time, end_time, title, description, color, task_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [date, startTime, endTime, title, description, color, taskId]);
        res.status(201).json({
            id: result.lastID,
...
router.put('/time-blocks/:id', [
    (0, express_validator_1.param)('id').isInt(),
    (0, express_validator_1.body)('startTime').optional().matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/),
    (0, express_validator_1.body)('endTime').optional().matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/),
    (0, express_validator_1.body)('title').optional().notEmpty(),
    (0, express_validator_1.body)('description').optional().isString(),
    (0, express_validator_1.body)('color').optional().isString(),
    (0, express_validator_1.body)('taskId').optional(),
    ...
    try {
        const existing = await (0, database_1.getAsync)('SELECT * FROM time_blocks WHERE id = ?', [blockId]);
        if (!existing) {
            return res.status(404).json({ error: 'Time block not found' });
        }
        await (0, database_1.runAsync)(`UPDATE time_blocks SET
        ...
        task_id = COALESCE(?, task_id),
        updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`, [startTime, endTime, title, description, color, taskId, blockId]);
 ...
 * Delete a time block
 */
router.delete('/time-blocks/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const result = await (0, database_1.runAsync)('DELETE FROM time_blocks WHERE id = ?', [req.params.id]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Time block not found' });
        }
...
router.get('/overview', [
    ...
    try {
        ...
        const [priorities, notes, reflection, timeBlocks, todayTasks] = await Promise.all([
            (0, database_1.allAsync)(`SELECT dp.*, t.title as task_title, t.column_id
         FROM daily_priorities dp
         LEFT JOIN tasks t ON dp.task_id = t.id
         WHERE dp.date = ?
         ORDER BY dp.position ASC`, [date]),
            (0, database_1.getAsync)('SELECT * FROM daily_notes WHERE date = ?', [date]),
            (0, database_1.getAsync)('SELECT * FROM daily_reflections WHERE date = ?', [date]),
            (0, database_1.allAsync)(`SELECT tb.*, t.title as task_title
         FROM time_blocks tb
         LEFT JOIN tasks t ON tb.task_id = t.id
         WHERE tb.date = ?
         ORDER BY tb.start_time ASC`, [date]),
            // Get tasks due today
            (0, database_1.allAsync)(`SELECT t.*, c.name as column_name, c.color as column_color
         FROM tasks t
         JOIN columns c ON t.column_id = c.id
         WHERE DATE(t.due_date) = ?
         ORDER BY t.priority DESC, t.position ASC`, [date]),
        ]);
        res.json({
            date,
