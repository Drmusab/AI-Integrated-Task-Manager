...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const inboxCount = await (0, database_1.getAsync)(`SELECT COUNT(*) as count FROM tasks WHERE gtd_status = 'inbox'`);
        // Get today's tasks (Do First quadrant + tasks due today)
        const todayTasks = await (0, database_1.allAsync)(`SELECT t.*, c.name as column_name, c.color as column_color, p.name as project_name
       FROM tasks t
       ...
       ORDER BY t.importance DESC, t.urgency DESC, t.priority DESC`, [today]);
        // Get this week's tasks
        const weekTasks = await (0, database_1.allAsync)(`SELECT t.*, c.name as column_name, c.color as column_color, p.name as project_name
       FROM tasks t
       LEFT JOIN columns c ON t.column_id = c.id
       LEFT JOIN projects p ON t.project_id = p.id
       WHERE t.execution_status = 'this_week'
          OR (DATE(t.due_date) BETWEEN DATE(?) AND DATE(?))
       ORDER BY t.due_date ASC, t.priority DESC`, [today, weekFromNow]);
        // Get GTD status distribution
        const gtdDistribution = await (0, database_1.allAsync)(`SELECT gtd_status, COUNT(*) as count FROM tasks GROUP BY gtd_status`);
        // Get Eisenhower quadrant distribution
        const tasks = await (0, database_1.allAsync)(`SELECT urgency, importance FROM tasks WHERE gtd_status != 'done'`);
        ...
        const projects = await (0, database_1.allAsync)(`SELECT p.*, 
       ...
       LIMIT 5`);
        ...
        const waitingFor = await (0, database_1.allAsync)(`SELECT t.*, c.name as column_name
       FROM tasks t
       LEFT JOIN columns c ON t.column_id = c.id
...
router.get('/gtd/:status', async (req, res) => {
    try {
        ...
        const tasks = await (0, database_1.allAsync)(`SELECT t.*, c.name as column_name, c.color as column_color, p.name as project_name
       FROM tasks t
       LEFT JOIN columns c ON t.column_id = c.id
       LEFT JOIN projects p ON t.project_id = p.id
       WHERE t.gtd_status = ?
       ORDER BY t.priority DESC, t.created_at DESC`, [status]);
        // Get tags for each task
        const tasksWithTags = await Promise.all(tasks.map(async (task) => {
            const tags = await (0, database_1.allAsync)(`SELECT tg.* FROM tags tg JOIN task_tags tt ON tg.id = tt.tag_id WHERE tt.task_id = ?`, [task.id]);
...
router.get('/eisenhower/:quadrant', async (req, res) => {
    try {
       ...
       ORDER BY t.due_date ASC, t.priority DESC`, [urgency, importance]);
        // Get tags for each task
        const tasksWithTags = await Promise.all(tasks.map(async (task) => {
            const tags = await (0, database_1.allAsync)(`SELECT tg.* FROM tags tg JOIN task_tags tt ON tg.id = tt.tag_id WHERE tt.task_id = ?`, [task.id]);
...
router.put('/process/:id', [
    (0, express_validator_1.body)('gtd_status').isIn(['next_actions', 'waiting_for', 'someday_maybe', 'reference', 'done']),
    (0, express_validator_1.body)('urgency').optional().isBoolean(),
    (0, express_validator_1.body)('importance').optional().isBoolean(),
    (0, express_validator_1.body)('context').optional().isString(),
    (0, express_validator_1.body)('energy_required').optional().isIn(['high', 'medium', 'low']),
    (0, express_validator_1.body)('time_estimate').optional().isInt({ min: 0 }),
    (0, express_validator_1.body)('delegated_to').optional().isString(),
    (0, express_validator_1.body)('project_id').optional().isInt(),
    (0, express_validator_1.body)('action_date').optional().isISO8601(),
], async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const { gtd_status, urgency, importance, context, energy_required, time_estimate, delegated_to, project_id, action_date, category } = req.body;
        ...
        values.push(id);
        await (0, database_1.runAsync)(`UPDATE tasks SET ${updates.join(', ')} WHERE id = ?`, values);
...
router.put('/execution/:id', [
    (0, express_validator_1.body)('execution_status').isIn(['backlog', 'today', 'this_week', 'in_progress', 'waiting', 'review', 'done']),
...
router.get('/kanban', async (req, res) => {
    try {
       ...
       LEFT JOIN projects p ON t.project_id = p.id
       ${whereClause}
       ORDER BY t.position ASC`, params);
...
router.get('/projects', async (req, res) => {
    try {
        ...
        const projects = await (0, database_1.allAsync)(query, params);
...
router.get('/projects/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const project = await (0, database_1.getAsync)(`SELECT * FROM projects WHERE id = ?`, [id]);
        if (!project) {
            return res.status(404).json({ error: 'Project not found' });
        }
        const tasks = await (0, database_1.allAsync)(`SELECT t.*, c.name as column_name
       FROM tasks t
       LEFT JOIN columns c ON t.column_id = c.id
       WHERE t.project_id = ?
       ORDER BY t.gtd_status ASC, t.priority DESC`, [id]);
...
router.post('/projects', [
    ...
    (0, express_validator_1.body)('target_date').optional().isISO8601(),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        ...
        const result = await (0, database_1.runAsync)(`INSERT INTO projects (name, description, goal, color, start_date, target_date, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [name, description || null, goal || null, color || '#3498db', start_date || null, target_date || null, created_by || null]);
        const project = await (0, database_1.getAsync)(`SELECT * FROM projects WHERE id = ?`, [result.lastID]);
...
router.put('/projects/:id', async (req, res) => {
    try {
        ...
        values.push(id);
        await (0, database_1.runAsync)(`UPDATE projects SET ${updates.join(', ')} WHERE id = ?`, values);
        const project = await (0, database_1.getAsync)(`SELECT * FROM projects WHERE id = ?`, [id]);
...
router.delete('/projects/:id', async (req, res) => {
    try {
        ...
        await (0, database_1.runAsync)(`UPDATE tasks SET project_id = NULL WHERE project_id = ?`, [id]);
        await (0, database_1.runAsync)(`DELETE FROM projects WHERE id = ?`, [id]);
...
router.get('/contacts', async (req, res) => {
    try {
        ...
        const contacts = await (0, database_1.allAsync)(query, params);
        res.json(contacts);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post('/contacts', [
    ...
    (0, express_validator_1.body)('organization').optional().isString(),
    (0, express_validator_1.body)('role').optional().isString(),
    (0, express_validator_1.body)('contact_type').optional().isIn(['personal', 'work', 'vendor']),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        ...
        const result = await (0, database_1.runAsync)(`INSERT INTO contacts (name, email, phone, organization, role, contact_type, notes, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [name, email || null, phone || null, organization || null, role || null, contact_type || 'personal', notes || null, created_by || null]);
        const contact = await (0, database_1.getAsync)(`SELECT * FROM contacts WHERE id = ?`, [result.lastID]);
...
router.put('/contacts/:id', async (req, res) => {
    try {
        ...
        values.push(id);
        await (0, database_1.runAsync)(`UPDATE contacts SET ${updates.join(', ')} WHERE id = ?`, values);
        const contact = await (0, database_1.getAsync)(`SELECT * FROM contacts WHERE id = ?`, [id]);
...
router.delete('/contacts/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await (0, database_1.runAsync)(`DELETE FROM contacts WHERE id = ?`, [id]);
...
router.get('/contexts', async (req, res) => {
    try {
        const contexts = await (0, database_1.allAsync)(`SELECT DISTINCT context FROM tasks WHERE context IS NOT NULL AND context != '' ORDER BY context`);
...
router.get('/context/:context', async (req, res) => {
    try {
       ...
       ORDER BY t.energy_required DESC, t.priority DESC`, [context]);
        res.json(tasks);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post('/capture', [
    ...
    try {
        ...
        const firstColumn = await (0, database_1.getAsync)(`SELECT c.id FROM columns c 
       ...
       LIMIT 1`);
        ...
        // Get next position
        const positionResult = await (0, database_1.getAsync)(`SELECT MAX(position) as maxPosition FROM tasks WHERE column_id = ?`, [firstColumn.id]);
        const position = (positionResult?.maxPosition || 0) + 1;
        const result = await (0, database_1.runAsync)(`INSERT INTO tasks (title, description, column_id, position, gtd_status, created_by)
       VALUES (?, ?, ?, ?, 'inbox', ?)`, [title, description || null, firstColumn.id, position, created_by || null]);
        const task = await (0, database_1.getAsync)(`SELECT * FROM tasks WHERE id = ?`, [result.lastID]);
...
router.get('/categories', async (req, res) => {
    try {
        const categories = await (0, database_1.allAsync)(`SELECT DISTINCT category FROM tasks WHERE category IS NOT NULL AND category != '' ORDER BY category`);
        // Default categories
...
//# sourceMappingURL=omniplanner.js.map