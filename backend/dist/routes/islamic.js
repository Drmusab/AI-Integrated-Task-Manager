 ...
 */
router.get('/prayers', [
    (0, express_validator_1.query)('startDate').notEmpty().withMessage('startDate is required'),
    (0, express_validator_1.query)('endDate').notEmpty().withMessage('endDate is required'),
], async (req, res) => {
    ...
    try {
        const prayers = await (0, database_1.allAsync)(`SELECT * FROM prayer_records 
       WHERE date >= ? AND date <= ?
       ORDER BY date ASC, 
       CASE prayer_type 
         WHEN 'fajr' THEN 1 
...
router.get('/prayers/today', async (req, res) => {
    try {
        ...
        const prayers = await (0, database_1.allAsync)(`SELECT * FROM prayer_records WHERE date = ?
...
router.post('/prayers/log', [
    ...
    (0, express_validator_1.body)('prayerType').isIn(PRAYER_TYPES).withMessage('Invalid prayer type'),
    (0, express_validator_1.body)('status').isIn(['pending', 'prayed', 'missed', 'qada']).withMessage('Invalid status'),
    (0, express_validator_1.body)('onTime').optional().isBoolean(),
    (0, express_validator_1.body)('withJamaah').optional().isBoolean(),
    (0, express_validator_1.body)('sunnahBefore').optional().isBoolean(),
    (0, express_validator_1.body)('sunnahAfter').optional().isBoolean(),
    ...
    const { date, prayerType, status, onTime = false, withJamaah = false, sunnahBefore = false, sunnahAfter = false, notes = '', } = req.body;
    try {
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO prayer_records 
       (date, prayer_type, status, on_time, with_jamaah, sunnah_before, sunnah_after, notes, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [date, prayerType, status, onTime ? 1 : 0, withJamaah ? 1 : 0, sunnahBefore ? 1 : 0, sunnahAfter ? 1 : 0, notes]);
        // If prayer was missed, optionally create a qada entry
        if (status === 'missed') {
            const existingQada = await (0, database_1.getAsync)('SELECT id FROM qada_prayers WHERE prayer_type = ? AND original_date = ?', [prayerType, date]);
            if (!existingQada) {
                await (0, database_1.runAsync)(`INSERT INTO qada_prayers (prayer_type, original_date, status, priority)
           VALUES (?, ?, 'pending', 1)`, [prayerType, date]);
            }
        }
...
router.get('/prayers/summary', [
    (0, express_validator_1.query)('startDate').optional(),
    (0, express_validator_1.query)('endDate').optional(),
], async (req, res) => {
    try {
        ...
        const prayers = await (0, database_1.allAsync)(`SELECT prayer_type, status, on_time, with_jamaah 
       FROM prayer_records 
...
router.get('/qada', async (req, res) => {
    try {
        const qadaPrayers = await (0, database_1.allAsync)(`SELECT * FROM qada_prayers 
 ...
 */
router.post('/qada/:id/complete', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const today = new Date().toISOString().split('T')[0];
        await (0, database_1.runAsync)(`UPDATE qada_prayers SET status = 'completed', completed_date = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [today, req.params.id]);
...
router.get('/quran', [
    ...
    try {
        ...
        const startDate = req.query.startDate || new Date(today.getFullYear(), today.getMonth(), 1).toISOString().split('T')[0];
        const endDate = req.query.endDate || today.toISOString().split('T')[0];
        const logs = await (0, database_1.allAsync)(`SELECT * FROM quran_logs 
       WHERE date >= ? AND date <= ?
       ORDER BY date DESC, created_at DESC`, [startDate, endDate]);
        res.json(logs.map(l => ({
            id: l.id,
            date: l.date,
...
router.post('/quran', [
    (0, express_validator_1.body)('date').notEmpty().withMessage('Date is required'),
    (0, express_validator_1.body)('recitationType').optional().isIn(['tilawah', 'tafsir', 'hifz', 'muraja']),
    (0, express_validator_1.body)('surahStart').optional().isString(),
    (0, express_validator_1.body)('surahEnd').optional().isString(),
    (0, express_validator_1.body)('ayahStart').optional().isInt({ min: 1 }),
    (0, express_validator_1.body)('ayahEnd').optional().isInt({ min: 1 }),
    (0, express_validator_1.body)('juz').optional().isInt({ min: 1, max: 30 }),
    (0, express_validator_1.body)('pages').optional().isFloat({ min: 0 }),
    (0, express_validator_1.body)('durationMinutes').optional().isInt({ min: 0 }),
    (0, express_validator_1.body)('notes').optional().isString(),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
    ...
    const { date, recitationType = 'tilawah', surahStart, surahEnd, ayahStart, ayahEnd, juz, pages, durationMinutes, notes = '', } = req.body;
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO quran_logs 
       (date, recitation_type, surah_start, surah_end, ayah_start, ayah_end, juz, pages, duration_minutes, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [date, recitationType, surahStart, surahEnd, ayahStart, ayahEnd, juz, pages, durationMinutes, notes]);
        res.status(201).json({
            id: result.lastID,
...
router.get('/quran/summary', async (req, res) => {
    try {
        ...
        const monthlyStats = await (0, database_1.getAsync)(`SELECT 
         COUNT(*) as sessions,
         COALESCE(SUM(pages), 0) as totalPages,
         COALESCE(SUM(duration_minutes), 0) as totalMinutes
       FROM quran_logs 
       WHERE date LIKE ?`, [`${thisMonth}%`]);
        const byType = await (0, database_1.allAsync)(`SELECT recitation_type, COUNT(*) as count, COALESCE(SUM(duration_minutes), 0) as minutes
       FROM quran_logs 
       WHERE date LIKE ?
       GROUP BY recitation_type`, [`${thisMonth}%`]);
        res.json({
            month: thisMonth,
...
router.get('/dhikr', [
    ...
    try {
        ...
        const logs = await (0, database_1.allAsync)(`SELECT * FROM dhikr_logs WHERE date = ? ORDER BY dhikr_type`, [date]);
        res.json(logs.map(l => ({
            id: l.id,
            date: l.date,
...
router.post('/dhikr/log', [
    (0, express_validator_1.body)('date').notEmpty().withMessage('Date is required'),
    (0, express_validator_1.body)('dhikrType').notEmpty().withMessage('Dhikr type is required'),
    (0, express_validator_1.body)('count').optional().isInt({ min: 0 }),
    (0, express_validator_1.body)('target').optional().isInt({ min: 0 }),
    ...
    const { date, dhikrType, count = 0, target = 0, completed = false, notes = '', } = req.body;
    try {
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO dhikr_logs 
       (date, dhikr_type, count, target, completed, notes, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [date, dhikrType, count, target, completed ? 1 : 0, notes]);
...
router.get('/fasting', [
    ...
    try {
        ...
        const startDate = req.query.startDate || new Date(today.getFullYear(), today.getMonth(), 1).toISOString().split('T')[0];
        const endDate = req.query.endDate || today.toISOString().split('T')[0];
        const records = await (0, database_1.allAsync)(`SELECT * FROM fasting_records 
       WHERE date >= ? AND date <= ?
...
router.post('/fasting', [
    (0, express_validator_1.body)('date').notEmpty().withMessage('Date is required'),
    (0, express_validator_1.body)('fastType').optional().isIn(['obligatory', 'voluntary', 'monday_thursday', 'white_days', 'arafah', 'ashura', 'shawwal', 'other']),
    (0, express_validator_1.body)('status').optional().isIn(['planned', 'completed', 'missed', 'broken']),
    (0, express_validator_1.body)('notes').optional().isString(),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
    ...
    const { date, fastType = 'voluntary', status = 'planned', notes = '', } = req.body;
    try {
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO fasting_records (date, fast_type, status, notes, updated_at)
       VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)`, [date, fastType, status, notes]);
...
router.get('/jumuah', [
    ...
    try {
        const date = req.query.date || new Date().toISOString().split('T')[0];
        const checklist = await (0, database_1.getAsync)(`SELECT * FROM jumuah_checklist WHERE date = ?`, [date]);
        if (checklist) {
            res.json({
                id: checklist.id,
                date: checklist.date,
...
router.post('/jumuah', [
    (0, express_validator_1.body)('date').notEmpty().withMessage('Date is required'),
    (0, express_validator_1.body)('ghusl').optional().isBoolean(),
    (0, express_validator_1.body)('earlyArrival').optional().isBoolean(),
    (0, express_validator_1.body)('sunnahPrayers').optional().isBoolean(),
    (0, express_validator_1.body)('surahKahf').optional().isBoolean(),
    (0, express_validator_1.body)('extraDua').optional().isBoolean(),
    (0, express_validator_1.body)('specialAdhkar').optional().isBoolean(),
    (0, express_validator_1.body)('notes').optional().isString(),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
    ...
    const { date, ghusl = false, earlyArrival = false, sunnahPrayers = false, surahKahf = false, extraDua = false, specialAdhkar = false, notes = '', } = req.body;
    try {
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO jumuah_checklist 
       (date, ghusl, early_arrival, sunnah_prayers, surah_kahf, extra_dua, special_adhkar, notes, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [date, ghusl ? 1 : 0, earlyArrival ? 1 : 0, sunnahPrayers ? 1 : 0, surahKahf ? 1 : 0, extraDua ? 1 : 0, specialAdhkar ? 1 : 0, notes]);
...
router.get('/dua', [
    ...
    try {
        ...
        const duas = await (0, database_1.allAsync)(query, params);
        res.json(duas.map(d => ({
            id: d.id,
            title: d.title,
            duaText: d.dua_text,
...
router.post('/dua', [
    ...
    (0, express_validator_1.body)('duaText').optional().isString(),
    (0, express_validator_1.body)('category').optional().isIn(['quran', 'sunnah', 'personal', 'other']),
    ...
    const { title, duaText = '', category = 'personal', isRecurring = false, notes = '', } = req.body;
    const today = new Date().toISOString().split('T')[0];
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO dua_journal (title, dua_text, category, is_recurring, date_logged, notes)
       VALUES (?, ?, ?, ?, ?, ?)`, [title, duaText, category, isRecurring ? 1 : 0, today, notes]);
 ...
 */
router.patch('/dua/:id/answered', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const today = new Date().toISOString().split('T')[0];
        await (0, database_1.runAsync)(`UPDATE dua_journal SET status = 'answered', date_answered = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [today, req.params.id]);
...
router.get('/reflection', [
    ...
    try {
        const date = req.query.date || new Date().toISOString().split('T')[0];
        const reflection = await (0, database_1.getAsync)(`SELECT * FROM spiritual_reflections WHERE date = ?`, [date]);
        if (reflection) {
            res.json({
                id: reflection.id,
...
router.post('/reflection', [
    (0, express_validator_1.body)('date').notEmpty().withMessage('Date is required'),
    (0, express_validator_1.body)('prayerFocus').optional().isString(),
    (0, express_validator_1.body)('gratitude').optional().isString(),
    (0, express_validator_1.body)('improvements').optional().isString(),
    (0, express_validator_1.body)('keyInsights').optional().isString(),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    const { date, prayerFocus = '', gratitude = '', improvements = '', keyInsights = '', } = req.body;
    try {
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO spiritual_reflections 
       (date, prayer_focus, gratitude, improvements, key_insights, updated_at)
       VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [date, prayerFocus, gratitude, improvements, keyInsights]);
...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        // Get today's prayers
        const prayers = await (0, database_1.allAsync)(`SELECT * FROM prayer_records WHERE date = ?`, [today]);
        const prayerStatus = {};
        PRAYER_TYPES.forEach(type => {
        ...
        // Get today's dhikr
        const dhikr = await (0, database_1.allAsync)(`SELECT * FROM dhikr_logs WHERE date = ?`, [today]);
        // Get today's Quran recitation
        const quranToday = await (0, database_1.getAsync)(`SELECT COALESCE(SUM(pages), 0) as pages, COALESCE(SUM(duration_minutes), 0) as minutes
       FROM quran_logs WHERE date = ?`, [today]);
        // Get pending qada prayers count
        const qadaCount = await (0, database_1.getAsync)(`SELECT COUNT(*) as count FROM qada_prayers WHERE status = 'pending'`);
        // Get fasting status for today
        const fasting = await (0, database_1.getAsync)(`SELECT * FROM fasting_records WHERE date = ?`, [today]);
        // Get Jumu'ah checklist if it's Friday
        let jumuahChecklist = null;
        if (dayOfWeek === 5) {
            jumuahChecklist = await (0, database_1.getAsync)(`SELECT * FROM jumuah_checklist WHERE date = ?`, [today]);
        }
...
router.get('/settings', async (req, res) => {
    try {
        ...
        const settings = await (0, database_1.getAsync)(`SELECT * FROM islamic_settings LIMIT 1`);
        if (settings) {
            res.json({
...
router.post('/settings', [
    (0, express_validator_1.body)('calculationMethod').optional().isString(),
    ...
    (0, express_validator_1.body)('reminderMinutes').optional().isInt({ min: 0, max: 60 }),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    const { calculationMethod = 'MWL', latitude, longitude, timezone, notificationsEnabled = true, reminderMinutes = 15, } = req.body;
    try {
        ...
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO islamic_settings 
       (id, calculation_method, latitude, longitude, timezone, notifications_enabled, reminder_minutes, updated_at)
       VALUES (1, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [calculationMethod, latitude, longitude, timezone, notificationsEnabled ? 1 : 0, reminderMinutes]);
