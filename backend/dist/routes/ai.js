...
const findColumnByName = async (name) => {
    return (0, database_1.getAsync)('SELECT * FROM columns WHERE LOWER(name) = LOWER(?) LIMIT 1', [name]);
};
const findTaskByTitle = async (title) => {
    return (0, database_1.getAsync)('SELECT * FROM tasks WHERE LOWER(title) = LOWER(?) LIMIT 1', [title]);
};
const getNextPosition = async (columnId) => {
    const row = await (0, database_1.getAsync)('SELECT MAX(position) as maxPosition FROM tasks WHERE column_id = ?', [columnId]);
...
const createTask = async ({ title, description = '', columnId, dueDate, priority = 'medium' }) => {
    ...
    const insertResult = await (0, database_1.runAsync)(`INSERT INTO tasks (title, description, column_id, position, priority, due_date)
     VALUES (?, ?, ?, ?, ?, ?)`, [title, description, columnId, position, priority, dueDate || null]);
    (0, history_1.recordTaskHistory)(insertResult.lastID, 'created', null, null, null);
    (0, eventBus_1.emitEvent)('task', 'created', { taskId: insertResult.lastID, columnId });
    return insertResult.lastID;
};
const moveTaskToColumn = async (taskId, columnId) => {
    const position = await getNextPosition(columnId);
    await (0, database_1.runAsync)('UPDATE tasks SET column_id = ?, position = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [columnId, position, taskId]);
    (0, history_1.recordTaskHistory)(taskId, 'moved', null, `column:${columnId}`, null);
    (0, eventBus_1.emitEvent)('task', 'moved', { taskId, columnId });
};
const updateTaskDueDate = async (taskId, dueDate) => {
    await (0, database_1.runAsync)('UPDATE tasks SET due_date = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [dueDate, taskId]);
    (0, history_1.recordTaskHistory)(taskId, 'updated', 'due_date', dueDate, null);
    (0, eventBus_1.emitEvent)('task', 'updated', { taskId, dueDate });
};
const updateTaskPriority = async (taskId, priority) => {
    await (0, database_1.runAsync)('UPDATE tasks SET priority = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [priority, taskId]);
    (0, history_1.recordTaskHistory)(taskId, 'updated', 'priority', priority, null);
    (0, eventBus_1.emitEvent)('task', 'updated', { taskId, priority });
};
...
router.post('/command', [(0, express_validator_1.body)('command').notEmpty().withMessage('Command text is required')], async (req, res) => {
    ...
    try {
        ...
        if (parsed.action === 'list') {
            ...
            const tasks = await (0, database_1.allAsync)(query, params);
            return res.json({
                action: 'list',
                success: true,
                count: tasks.length,
