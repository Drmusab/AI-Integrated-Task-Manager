...
router.get('/muscle-groups', async (req, res) => {
    try {
        const groups = await (0, database_1.allAsync)('SELECT * FROM muscle_groups ORDER BY parent_id NULLS FIRST, name ASC');
...
router.get('/exercises', async (req, res) => {
    ...
    `;
        ...
        const exercises = await (0, database_1.allAsync)(sql, params);
 ...
 */
router.get('/exercises/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const exercise = await (0, database_1.getAsync)(`SELECT e.*, mg.name as muscle_group_name 
       FROM exercises e 
       LEFT JOIN muscle_groups mg ON e.primary_muscle_group_id = mg.id
...
router.post('/exercises', [
    ...
    (0, express_validator_1.body)('primaryMuscleGroupId').optional().isInt(),
    (0, express_validator_1.body)('difficulty').optional().isIn(['beginner', 'intermediate', 'advanced'])
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO exercises (name, description, demonstration_url, primary_muscle_group_id, equipment, difficulty, custom_notes, is_custom)
       VALUES (?, ?, ?, ?, ?, ?, ?, 1)`, [name, description || '', demonstrationUrl || '', primaryMuscleGroupId || null, equipment || '', difficulty || 'intermediate', customNotes || '']);
        res.status(201).json({
            id: result.lastID,
            name,
 ...
 * Update an exercise
 */
router.put('/exercises/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    const { name, description, demonstrationUrl, primaryMuscleGroupId, equipment, difficulty, customNotes } = req.body;
    try {
        const existing = await (0, database_1.getAsync)('SELECT * FROM exercises WHERE id = ?', [exerciseId]);
        if (!existing) {
            return res.status(404).json({ error: 'Exercise not found' });
        }
        await (0, database_1.runAsync)(`UPDATE exercises SET
        name = COALESCE(?, name),
       ...
       WHERE id = ?`, [name, description, demonstrationUrl, primaryMuscleGroupId, equipment, difficulty, customNotes, exerciseId]);
 ...
 */
router.delete('/exercises/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const result = await (0, database_1.runAsync)('DELETE FROM exercises WHERE id = ? AND is_custom = 1', [req.params.id]);
        if (result.changes === 0) {
...
router.get('/templates', async (req, res) => {
    try {
        const templates = await (0, database_1.allAsync)('SELECT * FROM workout_templates ORDER BY is_active DESC, created_at DESC');
 ...
 */
router.get('/templates/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const template = await (0, database_1.getAsync)('SELECT * FROM workout_templates WHERE id = ?', [req.params.id]);
        if (!template) {
            return res.status(404).json({ error: 'Template not found' });
        }
        // Get workout days with exercises
        const days = await (0, database_1.allAsync)('SELECT * FROM workout_days WHERE template_id = ? ORDER BY day_order ASC', [req.params.id]);
        const daysWithExercises = await Promise.all(days.map(async (day) => {
            const exercises = await (0, database_1.allAsync)(`SELECT wde.*, e.name as exercise_name, e.equipment, mg.name as muscle_group
         FROM workout_day_exercises wde
         JOIN exercises e ON wde.exercise_id = e.id
         LEFT JOIN muscle_groups mg ON e.primary_muscle_group_id = mg.id
         WHERE wde.workout_day_id = ?
         ORDER BY wde.exercise_order ASC`, [day.id]);
            return {
                id: day.id,
                name: day.name,
...
router.post('/templates', [
    (0, express_validator_1.body)('name').notEmpty().withMessage('Name is required'),
    (0, express_validator_1.body)('frequency').optional().isInt({ min: 1, max: 7 }),
    (0, express_validator_1.body)('goal').optional().isIn(['strength', 'hypertrophy', 'endurance', 'general'])
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO workout_templates (name, description, frequency, duration_weeks, goal)
       VALUES (?, ?, ?, ?, ?)`, [name, description || '', frequency || 3, durationWeeks || null, goal || 'general']);
        res.status(201).json({
            id: result.lastID,
            name,
 ...
 */
router.put('/templates/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    try {
        const existing = await (0, database_1.getAsync)('SELECT * FROM workout_templates WHERE id = ?', [templateId]);
        if (!existing) {
            return res.status(404).json({ error: 'Template not found' });
        }
        await (0, database_1.runAsync)(`UPDATE workout_templates SET
        name = COALESCE(?, name),
        description = COALESCE(?, description),
        ...
        updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`, [name, description, frequency, durationWeeks, goal, isActive !== undefined ? (isActive ? 1 : 0) : null, templateId]);
 ...
 */
router.delete('/templates/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const result = await (0, database_1.runAsync)('DELETE FROM workout_templates WHERE id = ?', [req.params.id]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Template not found' });
        }
...
router.post('/templates/:templateId/days', [
    (0, express_validator_1.param)('templateId').isInt(),
    (0, express_validator_1.body)('name').notEmpty().withMessage('Name is required')
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
    ...
    try {
        const template = await (0, database_1.getAsync)('SELECT * FROM workout_templates WHERE id = ?', [templateId]);
        if (!template) {
            return res.status(404).json({ error: 'Template not found' });
        }
        // Get next day order
        const maxOrder = await (0, database_1.getAsync)('SELECT MAX(day_order) as max FROM workout_days WHERE template_id = ?', [templateId]);
        const dayOrder = (maxOrder?.max ?? -1) + 1;
        const result = await (0, database_1.runAsync)('INSERT INTO workout_days (template_id, name, day_order, notes) VALUES (?, ?, ?, ?)', [templateId, name, dayOrder, notes || '']);
        res.status(201).json({
            id: result.lastID,
            name,
            dayOrder,
...
router.post('/days/:dayId/exercises', [
    (0, express_validator_1.param)('dayId').isInt(),
    (0, express_validator_1.body)('exerciseId').isInt().withMessage('Exercise ID is required')
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        const day = await (0, database_1.getAsync)('SELECT * FROM workout_days WHERE id = ?', [dayId]);
        if (!day) {
            return res.status(404).json({ error: 'Workout day not found' });
        }
        // Get next exercise order
        const maxOrder = await (0, database_1.getAsync)('SELECT MAX(exercise_order) as max FROM workout_day_exercises WHERE workout_day_id = ?', [dayId]);
        const exerciseOrder = (maxOrder?.max ?? -1) + 1;
        const result = await (0, database_1.runAsync)(`INSERT INTO workout_day_exercises (workout_day_id, exercise_id, sets, target_reps, rest_seconds, notes, exercise_order)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [dayId, exerciseId, sets || 3, targetReps || '8-12', restSeconds || 90, notes || '', exerciseOrder]);
        res.status(201).json({
            id: result.lastID,
...
router.get('/sessions', async (req, res) => {
    ...
    `;
        ...
        const sessions = await (0, database_1.allAsync)(sql, params);
 ...
 */
router.get('/sessions/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const session = await (0, database_1.getAsync)(`SELECT ws.*, wt.name as template_name, wd.name as workout_day_name
       FROM workout_sessions ws
       LEFT JOIN workout_templates wt ON ws.template_id = wt.id
       LEFT JOIN workout_days wd ON ws.workout_day_id = wd.id
       WHERE ws.id = ?`, [req.params.id]);
        ...
        // Get exercise logs
        const logs = await (0, database_1.allAsync)(`SELECT el.*, e.name as exercise_name, e.equipment, mg.name as muscle_group
       FROM exercise_logs el
       JOIN exercises e ON el.exercise_id = e.id
       LEFT JOIN muscle_groups mg ON e.primary_muscle_group_id = mg.id
       WHERE el.session_id = ?
       ORDER BY el.id ASC`, [req.params.id]);
...
router.post('/sessions', [
    ...
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO workout_sessions (template_id, workout_day_id, date, start_time, notes, energy_level)
       VALUES (?, ?, ?, ?, ?, ?)`, [templateId || null, workoutDayId || null, date, startTime || null, notes || '', energyLevel || 3]);
        res.status(201).json({
            id: result.lastID,
            date,
 ...
 */
router.put('/sessions/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    try {
        const existing = await (0, database_1.getAsync)('SELECT * FROM workout_sessions WHERE id = ?', [sessionId]);
        if (!existing) {
            return res.status(404).json({ error: 'Session not found' });
        }
        await (0, database_1.runAsync)(`UPDATE workout_sessions SET
        ...
        updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`, [endTime, durationMinutes, notes, energyLevel, overallFeeling, sessionId]);
 ...
 */
router.delete('/sessions/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const result = await (0, database_1.runAsync)('DELETE FROM workout_sessions WHERE id = ?', [req.params.id]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Session not found' });
        }
...
router.post('/sessions/:sessionId/logs', [
    ...
    (0, express_validator_1.body)('exerciseId').isInt().withMessage('Exercise ID is required'),
    (0, express_validator_1.body)('setNumber').isInt({ min: 1 }).withMessage('Set number is required')
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        const session = await (0, database_1.getAsync)('SELECT * FROM workout_sessions WHERE id = ?', [sessionId]);
        if (!session) {
            return res.status(404).json({ error: 'Session not found' });
        }
        // Check for PR
        let isPR = false;
        if (weight && reps) {
            const previousBest = await (0, database_1.getAsync)(`SELECT MAX(weight) as max_weight FROM exercise_logs 
         WHERE exercise_id = ? AND reps >= ? AND session_id != ?`, [exerciseId, reps, sessionId]);
            if (!previousBest || !previousBest.max_weight || weight > previousBest.max_weight) {
                isPR = true;
                // Record the PR
                await (0, database_1.runAsync)(`INSERT INTO personal_records (exercise_id, record_type, value, reps, date, session_id)
           VALUES (?, 'weight', ?, ?, ?, ?)`, [exerciseId, weight, reps, session.date, sessionId]);
            }
        }
        const result = await (0, database_1.runAsync)(`INSERT INTO exercise_logs (session_id, exercise_id, set_number, reps, weight, rpe, rest_seconds, notes, is_pr)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [sessionId, exerciseId, setNumber, reps || null, weight || null, rpe || null, restSeconds || null, notes || '', isPR ? 1 : 0]);
        res.status(201).json({
            id: result.lastID,
            isPR,
 ...
 */
router.put('/logs/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    try {
        const existing = await (0, database_1.getAsync)('SELECT * FROM exercise_logs WHERE id = ?', [logId]);
        if (!existing) {
            return res.status(404).json({ error: 'Log not found' });
        }
        await (0, database_1.runAsync)(`UPDATE exercise_logs SET
        reps = COALESCE(?, reps),
        weight = COALESCE(?, weight),
        rpe = COALESCE(?, rpe),
        rest_seconds = COALESCE(?, rest_seconds),
        notes = COALESCE(?, notes)
       WHERE id = ?`, [reps, weight, rpe, restSeconds, notes, logId]);
...
router.get('/personal-records', async (req, res) => {
    ...
    `;
        ...
        const records = await (0, database_1.allAsync)(sql, params);
...
router.get('/measurements', async (req, res) => {
    try {
        ...
        const measurements = await (0, database_1.allAsync)(sql, params);
...
router.post('/measurements', [
    ...
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO body_measurements (date, weight, body_fat_percent, chest, waist, hips, left_arm, right_arm, left_thigh, right_thigh, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [date, weight || null, bodyFatPercent || null, chest || null, waist || null, hips || null, leftArm || null, rightArm || null, leftThigh || null, rightThigh || null, notes || '']);
        res.status(201).json({
            id: result.lastID,
            date,
...
router.get('/profile', async (req, res) => {
    try {
        ...
        const profile = await (0, database_1.getAsync)('SELECT * FROM fitness_profile LIMIT 1');
        if (!profile) {
...
router.post('/profile', async (req, res) => {
    ...
    try {
        ...
        const existing = await (0, database_1.getAsync)('SELECT * FROM fitness_profile LIMIT 1');
        if (existing) {
            await (0, database_1.runAsync)(`UPDATE fitness_profile SET
          height = COALESCE(?, height),
          age = COALESCE(?, age),
          gender = COALESCE(?, gender),
          activity_level = COALESCE(?, activity_level),
          updated_at = CURRENT_TIMESTAMP
         WHERE id = ?`, [height, age, gender, activityLevel, existing.id]);
            res.json({ message: 'Profile updated successfully' });
        }
        else {
            const result = await (0, database_1.runAsync)('INSERT INTO fitness_profile (height, age, gender, activity_level) VALUES (?, ?, ?, ?)', [height || null, age || null, gender || null, activityLevel || 'moderate']);
...
router.get('/goals', async (req, res) => {
    ...
    `;
        ...
        const goals = await (0, database_1.allAsync)(sql, params);
 ...
 */
router.post('/goals', [
    (0, express_validator_1.body)('goalType').notEmpty().withMessage('Goal type is required')
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        const result = await (0, database_1.runAsync)(`INSERT INTO fitness_goals (goal_type, target_value, target_description, deadline, exercise_id)
       VALUES (?, ?, ?, ?, ?)`, [goalType, targetValue || null, targetDescription || '', deadline || null, exerciseId || null]);
        res.status(201).json({
            id: result.lastID,
            goalType,
 ...
 */
router.put('/goals/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    try {
        const existing = await (0, database_1.getAsync)('SELECT * FROM fitness_goals WHERE id = ?', [goalId]);
        if (!existing) {
            return res.status(404).json({ error: 'Goal not found' });
        }
        await (0, database_1.runAsync)(`UPDATE fitness_goals SET
        ...
        updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`, [targetValue, targetDescription, deadline, currentProgress, status, goalId]);
 ...
 */
router.delete('/goals/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const result = await (0, database_1.runAsync)('DELETE FROM fitness_goals WHERE id = ?', [req.params.id]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Goal not found' });
        }
 ...
 */
router.get('/progress/exercise/:exerciseId', [(0, express_validator_1.param)('exerciseId').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        ...
        // Get exercise info
        const exercise = await (0, database_1.getAsync)('SELECT * FROM exercises WHERE id = ?', [exerciseId]);
        if (!exercise) {
            return res.status(404).json({ error: 'Exercise not found' });
        }
        // Get session data with max weight per session
        const progress = await (0, database_1.allAsync)(`SELECT ws.date, MAX(el.weight) as max_weight, MAX(el.reps) as max_reps
       FROM exercise_logs el
       JOIN workout_sessions ws ON el.session_id = ws.id
       WHERE el.exercise_id = ? AND ws.date >= date('now', '-' || ? || ' days')
       GROUP BY ws.date
       ORDER BY ws.date ASC`, [exerciseId, days]);
...
router.get('/stats/weekly', async (req, res) => {
    try {
        ...
        const sessions = await (0, database_1.allAsync)(`SELECT ws.*, 
       ...
       ORDER BY ws.date DESC`);
        ...
        const muscleVolume = await (0, database_1.allAsync)(`SELECT mg.name, SUM(el.weight * el.reps) as volume
       FROM exercise_logs el
       JOIN workout_sessions ws ON el.session_id = ws.id
       JOIN exercises e ON el.exercise_id = e.id
       LEFT JOIN muscle_groups mg ON e.primary_muscle_group_id = mg.id
...
router.get('/today', async (req, res) => {
    try {
        ...
        const todaySession = await (0, database_1.getAsync)(`SELECT ws.*, wt.name as template_name, wd.name as workout_day_name
       FROM workout_sessions ws
       LEFT JOIN workout_templates wt ON ws.template_id = wt.id
       LEFT JOIN workout_days wd ON ws.workout_day_id = wd.id
       WHERE ws.date = ?`, [today]);
        // Get active template
        const activeTemplate = await (0, database_1.getAsync)('SELECT * FROM workout_templates WHERE is_active = 1 LIMIT 1');
        // Get last session
        const lastSession = await (0, database_1.getAsync)(`SELECT ws.*, wt.name as template_name
       FROM workout_sessions ws
       LEFT JOIN workout_templates wt ON ws.template_id = wt.id
       WHERE ws.date < ?
       ORDER BY ws.date DESC LIMIT 1`, [today]);
        // Get this week's session count
        const weekSessionCount = await (0, database_1.getAsync)(`SELECT COUNT(*) as count FROM workout_sessions 
