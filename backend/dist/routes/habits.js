...
const habitValidationSchema = [
    ...
    (0, express_validator_1.body)('goalType').optional().isIn(['binary', 'numeric', 'timer']).withMessage('goalType must be binary, numeric, or timer'),
    (0, express_validator_1.body)('goalValue').optional().isInt({ min: 1 }).withMessage('goalValue must be a positive integer'),
    (0, express_validator_1.body)('goalUnit').optional().isString(),
    (0, express_validator_1.body)('frequency').optional().isIn(['daily', 'weekly']).withMessage('frequency must be daily or weekly'),
    (0, express_validator_1.body)('daysOfWeek').optional().isArray(),
...
router.get('/', async (req, res) => {
    try {
        ...
        const habits = await (0, database_1.allAsync)(`SELECT * FROM habits ${whereClause} ORDER BY position ASC, created_at DESC`);
...
router.get('/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    ...
    try {
        const habit = await (0, database_1.getAsync)('SELECT * FROM habits WHERE id = ?', [req.params.id]);
        if (!habit) {
...
router.post('/', habitValidationSchema, async (req, res) => {
    ...
    const { name, description = '', category = 'general', goalType = 'binary', goalValue = 1, goalUnit = '', frequency = 'daily', daysOfWeek = null, color = '#3498db', icon = null, } = req.body;
    try {
        // Get max position
        const maxPosition = await (0, database_1.getAsync)('SELECT MAX(position) as max FROM habits WHERE archived = 0');
        const position = (maxPosition?.max ?? -1) + 1;
        const result = await (0, database_1.runAsync)(`INSERT INTO habits (name, description, category, goal_type, goal_value, goal_unit, frequency, days_of_week, color, icon, position)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [name, description, category, goalType, goalValue, goalUnit, frequency, daysOfWeek ? JSON.stringify(daysOfWeek) : null, color, icon, position]);
        res.status(201).json({
            id: result.lastID,
            name,
            description,
 ...
 */
router.put('/:id', [(0, express_validator_1.param)('id').isInt(), ...habitValidationSchema], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    const habitId = Number(req.params.id);
    const { name, description, category, goalType, goalValue, goalUnit, frequency, daysOfWeek, color, icon, position, archived, } = req.body;
    try {
        const existingHabit = await (0, database_1.getAsync)('SELECT * FROM habits WHERE id = ?', [habitId]);
        if (!existingHabit) {
            return res.status(404).json({ error: 'Habit not found' });
        }
        await (0, database_1.runAsync)(`UPDATE habits SET
        name = COALESCE(?, name),
       ...
       WHERE id = ?`, [name, description, category, goalType, goalValue, goalUnit, frequency,
 ...
 * Delete a habit
 */
router.delete('/:id', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    const habitId = Number(req.params.id);
    try {
        const result = await (0, database_1.runAsync)('DELETE FROM habits WHERE id = ?', [habitId]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Habit not found' });
        }
 ...
 */
router.patch('/:id/archive', [(0, express_validator_1.param)('id').isInt(), (0, express_validator_1.body)('archived').isBoolean()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    try {
        ...
        await (0, database_1.runAsync)('UPDATE habits SET archived = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [archived ? 1 : 0, habitId]);
...
router.post('/:id/log', [
    ...
    (0, express_validator_1.body)('status').isIn(['done', 'missed', 'skipped', 'pending']).withMessage('Invalid status'),
    (0, express_validator_1.body)('value').optional().isFloat({ min: 0 }),
    ...
    try {
        const habit = await (0, database_1.getAsync)('SELECT * FROM habits WHERE id = ?', [habitId]);
        if (!habit) {
            return res.status(404).json({ error: 'Habit not found' });
        }
        // Use REPLACE to upsert the log
        await (0, database_1.runAsync)(`INSERT OR REPLACE INTO habit_logs (habit_id, log_date, status, value, notes, updated_at)
       VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [habitId, date, status, value, notes]);
...
router.get('/logs/range', [
    ...
    try {
        const logs = await (0, database_1.allAsync)(`SELECT hl.*, h.name as habit_name, h.goal_type, h.goal_value, h.color
       FROM habit_logs hl
       JOIN habits h ON hl.habit_id = h.id
       WHERE hl.log_date >= ? AND hl.log_date <= ?
       ORDER BY hl.log_date ASC`, [startDate, endDate]);
 ...
 */
router.get('/:id/stats', [(0, express_validator_1.param)('id').isInt()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    try {
        ...
        const logs = await (0, database_1.allAsync)(`SELECT log_date, status FROM habit_logs 
       WHERE habit_id = ? AND status = 'done'
       ORDER BY log_date DESC`, [habitId]);
        ...
        const monthlyLogs = await (0, database_1.allAsync)(`SELECT status FROM habit_logs 
       WHERE habit_id = ? AND log_date LIKE ?`, [habitId, `${thisMonth}%`]);
...
router.get('/summary/weekly', async (req, res) => {
    try {
        ...
        const habits = await (0, database_1.allAsync)('SELECT * FROM habits WHERE archived = 0 ORDER BY position ASC');
        // Get all logs for this week
        const logs = await (0, database_1.allAsync)(`SELECT * FROM habit_logs 
       WHERE log_date >= ? AND log_date <= ?`, [startDateStr, endDateStr]);
...
router.get('/summary/monthly', [
    ...
    (0, express_validator_1.query)('year').optional().isInt(),
], async (req, res) => {
    try {
        const now = new Date();
       ...
       WHERE log_date >= ? AND log_date <= ?`, [startDateStr, endDateStr]);
        ...
        const habitStats = await Promise.all(habits.map(async (habit) => {
            ...
            // Get streak info
            const allLogs = await (0, database_1.allAsync)(`SELECT log_date, status FROM habit_logs 
         WHERE habit_id = ? AND status = 'done'
         ORDER BY log_date DESC`, [habit.id]);
            // Calculate current streak
            let currentStreak = 0;
            let longestStreak = 0;
            let tempStreak = 0;
 ...
 */
router.put('/reorder', [(0, express_validator_1.body)('habitIds').isArray()], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    ...
    try {
        for (let i = 0; i < habitIds.length; i++) {
            await (0, database_1.runAsync)('UPDATE habits SET position = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [i, habitIds[i]]);
        }
