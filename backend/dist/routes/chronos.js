...
const chronos_1 = require("../services/chronos");
 ...
 */
router.get('/time-blocks', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
    ...
    `;
        ...
        const blocks = await (0, database_1.allAsync)(query, params);
        res.json(blocks);
    }
    catch (error) {
 ...
 */
router.get('/time-blocks/:id', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const block = await (0, database_1.getAsync)('SELECT * FROM chronos_time_blocks WHERE id = ? AND created_by = ?', [id, userId]);
        if (!block) {
 ...
 */
router.post('/time-blocks', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { title, description, date, start_time, end_time, color, category, task_id, project_id, habit_id, is_template, template_name, recurrence_rule, buffer_before, buffer_after, energy_required, focus_level, location, notes } = req.body;
        const userId = req.user.id;
        ...
        const settings = await (0, database_1.getAsync)('SELECT conflict_warnings_enabled FROM chronos_settings WHERE user_id = ?', [userId]);
        if (!settings || settings.conflict_warnings_enabled) {
            const conflicts = await (0, database_1.allAsync)(`
      ...
      `, [userId, date, end_time, start_time, end_time, start_time, start_time, end_time]);
        ...
        const result = await (0, database_1.runAsync)(`
    ...
    `, [
        ...
        const newBlock = await (0, database_1.getAsync)('SELECT * FROM chronos_time_blocks WHERE id = ?', [result.lastID]);
 ...
 */
router.put('/time-blocks/:id', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        // Verify ownership
        const existingBlock = await (0, database_1.getAsync)('SELECT * FROM chronos_time_blocks WHERE id = ? AND created_by = ?', [id, userId]);
        if (!existingBlock) {
            return res.status(404).json({ error: 'Time block not found' });
        }
        const { title, description, date, start_time, end_time, color, category, task_id, project_id, habit_id, buffer_before, buffer_after, energy_required, focus_level, location, notes } = req.body;
        await (0, database_1.runAsync)(`
    ...
    `, [
        ...
        const updatedBlock = await (0, database_1.getAsync)('SELECT * FROM chronos_time_blocks WHERE id = ?', [id]);
 ...
 */
router.delete('/time-blocks/:id', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const result = await (0, database_1.runAsync)('DELETE FROM chronos_time_blocks WHERE id = ? AND created_by = ?', [id, userId]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Time block not found' });
        }
 ...
 */
router.get('/time-sessions', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
    ...
    `;
        ...
        const sessions = await (0, database_1.allAsync)(query, params);
        res.json(sessions);
    }
    catch (error) {
 ...
 */
router.get('/time-sessions/active', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const session = await (0, database_1.getAsync)(`
 ...
 */
router.post('/time-sessions/start', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { time_block_id, task_id, project_id, title, description, category, is_pomodoro } = req.body;
        const userId = req.user.id;
        if (!title) {
            return res.status(400).json({ error: 'Title is required' });
        }
        // Check if there's already an active session
        const activeSession = await (0, database_1.getAsync)('SELECT id FROM chronos_time_sessions WHERE created_by = ? AND status = ?', [userId, 'active']);
        if (activeSession) {
            return res.status(409).json({
    ...
    `, [
        ...
        const newSession = await (0, database_1.getAsync)('SELECT * FROM chronos_time_sessions WHERE id = ?', [result.lastID]);
 ...
 */
router.post('/time-sessions/:id/pause', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const session = await (0, database_1.getAsync)('SELECT * FROM chronos_time_sessions WHERE id = ? AND created_by = ?', [id, userId]);
        if (!session) {
      ...
      WHERE id = ?
    `, [id]);
        const updatedSession = await (0, database_1.getAsync)('SELECT * FROM chronos_time_sessions WHERE id = ?', [id]);
 ...
 */
router.post('/time-sessions/:id/resume', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const session = await (0, database_1.getAsync)('SELECT * FROM chronos_time_sessions WHERE id = ? AND created_by = ?', [id, userId]);
        ...
        const pauseDuration = await (0, database_1.getAsync)(`
 ...
 */
router.post('/time-sessions/:id/stop', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { energy_level, focus_quality, productivity_rating, interruptions, notes } = req.body;
        const userId = req.user.id;
        const session = await (0, database_1.getAsync)('SELECT * FROM chronos_time_sessions WHERE id = ? AND created_by = ?', [id, userId]);
        if (!session) {
            return res.status(404).json({ error: 'Session not found' });
        ...
        const durationQuery = await (0, database_1.getAsync)(`
      SELECT (julianday('now') - julianday(?)) * 24 * 60 as minutes
    `, [session.start_time]);
 ...
 */
router.get('/analytics/weekly', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const plannedVsActual = await (0, database_1.allAsync)(`
      SELECT
    ...
    `, [userId, weekStart, weekEnd]);
        // Category breakdown
        const categoryBreakdown = await (0, database_1.allAsync)(`
      SELECT
        category,
    ...
    `, [userId, weekStart, weekEnd]);
        ...
        const energyPatterns = await (0, database_1.allAsync)(`
      SELECT
    ...
    `, [userId, weekStart, weekEnd]);
        // Focus analysis
        const focusAnalysis = await (0, database_1.getAsync)(`
      SELECT
 ...
 */
router.get('/analytics/productivity-score', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { days = 7 } = req.query;
        const startDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        const score = await (0, database_1.getAsync)(`
      SELECT
        COUNT(DISTINCT tb.id) as planned_blocks,
    ...
    `, [userId, startDate]);
        const completionRate = score.planned_blocks > 0
            ...
            : 0;
        const productivityScore = Math.round((completionRate * 0.4) +
 ...
 */
router.get('/templates', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const templates = await (0, database_1.allAsync)('SELECT * FROM chronos_time_templates WHERE created_by = ? ORDER BY name', [userId]);
        res.json(templates);
    }
    catch (error) {
 ...
 */
router.post('/templates', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { name, description, title, start_time, duration, color, category, recurrence_pattern, days_of_week, buffer_before, buffer_after, energy_required, focus_level } = req.body;
        const userId = req.user.id;
    ...
    `, [
        ...
        const newTemplate = await (0, database_1.getAsync)('SELECT * FROM chronos_time_templates WHERE id = ?', [result.lastID]);
 ...
 */
router.delete('/templates/:id', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const result = await (0, database_1.runAsync)('DELETE FROM chronos_time_templates WHERE id = ? AND created_by = ?', [id, userId]);
        if (result.changes === 0) {
 ...
 */
router.get('/settings', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        let settings = await (0, database_1.getAsync)('SELECT * FROM chronos_settings WHERE user_id = ?', [userId]);
        // Create default settings if none exist
        if (!settings) {
            await (0, database_1.runAsync)('INSERT INTO chronos_settings (user_id) VALUES (?)', [userId]);
            settings = await (0, database_1.getAsync)('SELECT * FROM chronos_settings WHERE user_id = ?', [userId]);
        }
        res.json(settings);
    }
    catch (error) {
 ...
 */
router.put('/settings', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { work_hours_start, work_hours_end, default_block_duration, default_break_duration, pomodoro_work_duration, pomodoro_break_duration, pomodoro_long_break, auto_schedule_enabled, conflict_warnings_enabled, buffer_time_enabled, default_buffer_minutes, focus_mode_default, break_reminders_enabled, idle_timeout_minutes, timezone } = req.body;
        // Ensure settings exist
        const existingSettings = await (0, database_1.getAsync)('SELECT * FROM chronos_settings WHERE user_id = ?', [userId]);
        if (!existingSettings) {
            await (0, database_1.runAsync)('INSERT INTO chronos_settings (user_id) VALUES (?)', [userId]);
        }
        // Update settings
    ...
    `, [
        ...
        const updatedSettings = await (0, database_1.getAsync)('SELECT * FROM chronos_settings WHERE user_id = ?', [userId]);
 ...
 */
router.post('/pomodoro/start', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { task_id, duration, break_duration } = req.body;
        const userId = req.user.id;
        // Get user's Pomodoro settings
        const settings = await (0, database_1.getAsync)('SELECT pomodoro_work_duration, pomodoro_break_duration FROM chronos_settings WHERE user_id = ?', [userId]);
    ...
    `, [task_id || null, workDuration, breakDur, userId]);
        const newPomodoro = await (0, database_1.getAsync)('SELECT * FROM chronos_pomodoro_sessions WHERE id = ?', [result.lastID]);
 ...
 */
router.post('/pomodoro/:id/complete', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        await (0, database_1.runAsync)(`
      ...
      WHERE id = ? AND created_by = ?
    `, [id, userId]);
        const updatedPomodoro = await (0, database_1.getAsync)('SELECT * FROM chronos_pomodoro_sessions WHERE id = ?', [id]);
 ...
 */
router.post('/integrate/task-to-block', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Get task details
        const task = await (0, database_1.getAsync)('SELECT * FROM tasks WHERE id = ? AND created_by = ?', [task_id, userId]);
        if (!task) {
            return res.status(404).json({ error: 'Task not found' });
        }
        // Calculate end time
        const blockDuration = duration || task.time_estimate || 60;
        const end_time = (0, chronos_1.addMinutes)(start_time, blockDuration);
        // Create time block
        const result = await (0, database_1.runAsync)(`
      INSERT INTO chronos_time_blocks (
 ...
 */
router.post('/integrate/auto-schedule-tasks', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        ...
        for (const taskId of task_ids) {
            const task = await (0, database_1.getAsync)('SELECT * FROM tasks WHERE id = ? AND created_by = ?', [taskId, userId]);
            if (task) {
                try {
                    const suggestion = await (0, chronos_1.autoScheduleTask)(userId, {
 ...
 */
router.get('/integrate/daily-blocks/:date', jwtAuth_1.authenticateToken, async (req, res) => {
    try {
        const { date } = req.params;
        const userId = req.user.id;
        // Get time blocks for the date
        const blocks = await (0, database_1.allAsync)(`
    ...
    `, [userId, date]);
        ...
        const priorities = await (0, database_1.allAsync)('SELECT * FROM daily_priorities WHERE created_by = ? AND date = ? ORDER BY position', [userId, date]);
        res.json({
            date,
            blocks,
            priorities,
            totalPlannedMinutes: blocks.reduce((sum, block) => {
                return sum + (0, chronos_1.getMinutesDifference)(block.start_time, block.end_time);
            }, 0)
        });
    }
    catch (error) {
