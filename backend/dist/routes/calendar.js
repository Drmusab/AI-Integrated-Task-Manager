...
const calendarEventsValidations = [
    ...
    (0, express_validator_1.query)('boardId')
        .optional()
...
router.get('/events', calendarEventsValidations, async (req, res) => {
    ...
    `;
        ...
        // Execute the query
        database_1.db.all(query, params, async (err, rows) => {
            if (err) {
            ...
            try {
                ...
                const [tagsResults, subtasksResults] = await Promise.all([
                    new Promise((resolve, reject) => {
                        database_1.db.all(tagsQuery, taskIds, (err, tagRows) => {
                            if (err)
                                reject(err);
                            else
                                resolve(tagRows || []);
                    ...
                    new Promise((resolve, reject) => {
                        database_1.db.all(subtasksQuery, taskIds, (err, subtaskRows) => {
                            if (err)
                                reject(err);
                            else
                                resolve(subtaskRows || []);
