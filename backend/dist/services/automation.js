...
const triggerAutomation = async (eventType, eventData) => {
    try {
        const rules = await (0, database_1.allAsync)('SELECT * FROM automation_rules WHERE trigger_type = ? AND enabled = 1', [eventType]);
        for (const rule of rules) {
            try {
                ...
                await (0, database_1.runAsync)('INSERT INTO automation_logs (rule_id, status, message) VALUES (?, ?, ?)', [rule.id, status, message]);
            }
            catch (error) {
                logger.error(`Error executing automation rule ${rule.id}`, { error: error.message });
                try {
                    await (0, database_1.runAsync)('INSERT INTO automation_logs (rule_id, status, message) VALUES (?, ?, ?)', [rule.id, 'failed', error.message]);
                }
                catch (logError) {
...
const executeAutomationAction = async (actionType, actionConfig, eventData) => {
    switch (actionType) {
        case 'webhook':
            return (0, webhook_1.triggerWebhook)(actionConfig.webhookId, eventData);
        case 'notification':
            return (0, notifications_1.sendNotification)(actionConfig.title || 'Automation Triggered', actionConfig.message || `Automation triggered by event: ${JSON.stringify(eventData)}`);
        case 'move_task': {
            ...
            const result = await (0, database_1.runAsync)('UPDATE tasks SET column_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [actionConfig.columnId, eventData.taskId]);
            if (result.changes === 0) {
        ...
        case 'update_task': {
            ...
            const result = await (0, database_1.runAsync)(`UPDATE tasks SET ${updates.join(', ')} WHERE id = ?`, values);
            if (result.changes === 0) {
                return { success: false, error: 'Task not found' };
            }
        ...
        case 'create_task': {
            ...
            const columnPosition = await (0, database_1.getAsync)('SELECT MAX(position) as maxPosition FROM tasks WHERE column_id = ?', [columnId]);
            const position = ((columnPosition && columnPosition.maxPosition) || 0) + 1;
            const result = await (0, database_1.runAsync)(`INSERT INTO tasks (title, description, column_id, position, priority, due_date, created_by, assigned_to)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                actionConfig.title,
