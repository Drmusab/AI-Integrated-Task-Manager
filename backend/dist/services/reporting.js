...
const generateWeeklyReport = async () => {
    ...
    try {
        ...
        const createdTasks = await (0, database_1.allAsync)('SELECT COUNT(*) as count FROM tasks WHERE created_at >= ?', [sinceISO]);
        // Tasks completed this week
        const completedTasks = await (0, database_1.allAsync)(`SELECT COUNT(*) as count
       FROM tasks t
       JOIN columns c ON t.column_id = c.id
       WHERE LOWER(c.name) = 'done' AND t.updated_at >= ?`, [sinceISO]);
        // Overdue tasks
        const overdueTasks = await (0, database_1.allAsync)(`SELECT COUNT(*) as count FROM tasks 
       WHERE due_date IS NOT NULL 
       AND due_date < ? 
       AND column_id NOT IN (SELECT id FROM columns WHERE LOWER(name) = 'done')`, [now]);
        // Tasks by column
        const tasksByColumn = await (0, database_1.allAsync)(`SELECT c.name as column, COUNT(t.id) as count
       FROM columns c
       LEFT JOIN tasks t ON t.column_id = c.id
       GROUP BY c.id
       ORDER BY c.position ASC`);
        // Tasks by priority
        const tasksByPriority = await (0, database_1.allAsync)(`SELECT priority, COUNT(*) as count
       FROM tasks
       WHERE column_id NOT IN (SELECT id FROM columns WHERE LOWER(name) = 'done')
       GROUP BY priority`);
        // Average completion time (tasks completed this week)
        const completionTimes = await (0, database_1.allAsync)(`SELECT 
       ...
       WHERE LOWER(c.name) = 'done' AND t.updated_at >= ?`, [sinceISO]);
        ...
        const activeBoards = await (0, database_1.allAsync)(`SELECT 
        b.id,
        b.name,
       ...
       LIMIT 5`, [sinceISO]);
        const report = {
            ...
            summary: {
                ...
                completionRate: calculateCompletionRate(completedTasks?.[0]?.count || 0, createdTasks?.[0]?.count || 0),
...
const generateCustomReport = async (startDate, endDate) => {
    ...
    try {
        const createdTasks = await (0, database_1.allAsync)('SELECT COUNT(*) as count FROM tasks WHERE created_at >= ? AND created_at <= ?', [start, end]);
        const completedTasks = await (0, database_1.allAsync)(`SELECT COUNT(*) as count
       FROM tasks t
       JOIN columns c ON t.column_id = c.id
       WHERE LOWER(c.name) = 'done' AND t.updated_at >= ? AND t.updated_at <= ?`, [start, end]);
        const tasksByColumn = await (0, database_1.allAsync)(`SELECT c.name as column, COUNT(t.id) as count
       FROM columns c
       LEFT JOIN tasks t ON t.column_id = c.id AND t.created_at >= ? AND t.created_at <= ?
       GROUP BY c.id
       ORDER BY c.position ASC`, [start, end]);
        return {
            period: {
                start,
                end,
...
const sendReportToN8n = async (report) => {
    try {
        const integrations = await (0, database_1.allAsync)('SELECT * FROM integrations WHERE type = ? AND enabled = 1', ['n8n_webhook']);
        ...
        const results = await Promise.allSettled(integrations.map(integration => (0, webhook_1.triggerWebhook)(integration.id, reportPayload)));
...
const generateProductivityAnalytics = async (days = 30) => {
    ...
    try {
        ...
        const dailyCompletions = await (0, database_1.allAsync)(`SELECT 
       ...
       ORDER BY date ASC`, [sinceISO]);
        // User productivity (if assigned_to is being used)
        const userProductivity = await (0, database_1.allAsync)(`SELECT 
        assigned_to,
       ...
       ORDER BY tasks_completed DESC`, [sinceISO]);
        // Task velocity (tasks completed per week)
        const velocity = await (0, database_1.allAsync)(`SELECT 
       ...
       WHERE LOWER(c.name) = 'done' AND t.updated_at >= ?
       GROUP BY week_ago
       ORDER BY week_ago ASC`, [sinceISO]);
        return {
            period: {
                days,
                start: sinceISO,
