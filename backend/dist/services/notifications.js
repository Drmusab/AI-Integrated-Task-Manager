...
const sendNotification = async (title, message, options = {}) => {
    try {
        const { type = 'info', taskId = null, boardId = null, priority = 'normal', sendToN8n = true, metadata = {} } = options;
        ...
        if (sendToN8n) {
            try {
                ...
                if (integrations && integrations.length > 0) {
                    ...
                    const webhookPromises = integrations.map(integration => (0, webhook_1.triggerWebhook)(integration.id, notificationPayload));
