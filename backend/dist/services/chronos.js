...
async function checkConflicts(userId, date, startTime, endTime, excludeBlockId = null) {
  ...
  `;
    ...
    return await (0, database_1.allAsync)(query, params);
}
/**
...
async function calculateBufferTime(userId, category) {
    const settings = await (0, database_1.getAsync)('SELECT buffer_time_enabled, default_buffer_minutes FROM chronos_settings WHERE user_id = ?', [userId]);
...
async function findAvailableSlots(userId, date, duration) {
    ...
    const settings = await (0, database_1.getAsync)('SELECT work_hours_start, work_hours_end FROM chronos_settings WHERE user_id = ?', [userId]);
    ...
    const existingBlocks = await (0, database_1.allAsync)(`
...
async function autoScheduleTask(userId, taskDetails, preferredDate = null) {
    ...
    while (attempts < maxAttempts) {
        ...
        if (availableSlots.length > 0) {
            ...
            let minDiff = Math.abs(parseInt(availableSlots[0].start_time.split(':')[0]) - optimalTime.hour);
...
async function generateDailyInsights(userId, date) {
    const blocks = await (0, database_1.allAsync)('SELECT * FROM chronos_time_blocks WHERE created_by = ? AND date = ? ORDER BY start_time', [userId, date]);
    const sessions = await (0, database_1.allAsync)(`
