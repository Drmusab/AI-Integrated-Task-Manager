...
const createRecurringTask = async (originalTask, recurringRule) => {
    ...
    if (isPositiveInteger(maxOccurrences)) {
        ...
        const occurrencesRow = await (0, database_1.getAsync)('SELECT COUNT(*) as count FROM tasks WHERE recurring_rule = ?', [ruleToMatch]);
    ...
    const positionRow = await (0, database_1.getAsync)('SELECT MAX(position) as maxPosition FROM tasks WHERE column_id = ?', [originalTask.column_id]);
    ...
    const insertResult = await (0, database_1.runAsync)(`INSERT INTO tasks (title, description, column_id, position, priority, due_date, recurring_rule, created_by, assigned_to)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        originalTask.title,
    ...
    (0, history_1.recordTaskHistory)(taskId, 'created', null, null, originalTask.created_by);
    // Copy tags from original task to new task instance
    const tagRows = await (0, database_1.allAsync)('SELECT tag_id FROM task_tags WHERE task_id = ?', [originalTask.id]);
    if (tagRows.length > 0) {
        await Promise.all(tagRows.map(tagRow => (0, database_1.runAsync)('INSERT INTO task_tags (task_id, tag_id) VALUES (?, ?)', [taskId, tagRow.tag_id])));
    }
    return taskId;
};
