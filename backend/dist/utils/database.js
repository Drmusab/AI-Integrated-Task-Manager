        ...
        db.serialize(async () => {
          ...
          FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE
                ...
                if (!userCount || userCount.count === 0) {
                    ...
                    await runAsync('INSERT INTO users (username, email, password_hash, role) VALUES (?, ?, ?, ?)', ['demo', 'demo@example.com', passwordHash, 'admin']);
                }
                ...
                if (!boardCount || boardCount.count === 0) {
                    const boardResult = await runAsync('INSERT INTO boards (name, description, template) VALUES (?, ?, ?)', ['Default Board', 'A basic Kanban board', 0]);
                    ...
                    for (const column of columns) {
                        await runAsync('INSERT INTO columns (board_id, name, color, icon, position) VALUES (?, ?, ?, ?, ?)', [boardId, column.name, column.color, column.icon, column.position]);
                    }
                    const tags = [
                ...
                if (!settingsCount || settingsCount.count === 0) {
                    ...
                    for (const setting of defaultSettings) {
                        await runAsync('INSERT INTO settings (key, value, description) VALUES (?, ?, ?)', [setting.key, setting.value, setting.description]);
                    }
                }
                ...
                if (!muscleGroupCount || muscleGroupCount.count === 0) {
                    ...
                    for (const group of mainGroups) {
                        const result = await runAsync('INSERT INTO muscle_groups (name, description) VALUES (?, ?)', [group.name, group.description]);
                    ...
                    for (const sub of subGroups) {
                        await runAsync('INSERT INTO muscle_groups (name, parent_id, description) VALUES (?, ?, ?)', [sub.name, groupIds[sub.parent], `Part of ${sub.parent}`]);
                    }
                    // Default exercises
                    ...
                    for (const ex of defaultExercises) {
                        await runAsync('INSERT INTO exercises (name, primary_muscle_group_id, equipment, difficulty, is_custom) VALUES (?, ?, ?, ?, 0)', [ex.name, groupIds[ex.muscle], ex.equipment, ex.difficulty]);
                    }
                }
                resolve();
            }
