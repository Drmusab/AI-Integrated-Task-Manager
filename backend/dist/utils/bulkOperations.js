...
async function bulkUpdateTasks(taskIds, updates, userId = null) {
  ...
  `;
    try {
        const result = await (0, database_1.runAsync)(sql, params);
        // Record history for each task
        if (userId) {
            for (const taskId of taskIds) {
                for (const [field, value] of Object.entries(updates)) {
                    if (allowedFields.includes(field)) {
                        await (0, history_1.recordTaskHistory)(taskId, 'bulk_update', field, String(value), userId);
                    }
                }
            }
        }
        ...
        for (const taskId of taskIds) {
            (0, eventBus_1.emitEvent)('task_updated', { taskId, updates });
        }
        // Trigger automation
        for (const taskId of taskIds) {
            await (0, automation_1.triggerAutomation)('task_updated', { taskId, ...updates });
        }
        return {
...
async function bulkDeleteTasks(taskIds, userId = null) {
    ...
    try {
        ...
        if (userId) {
            for (const taskId of taskIds) {
                await (0, history_1.recordTaskHistory)(taskId, 'deleted', 'status', 'deleted', userId);
            }
        }
        // Delete tasks (cascade will handle related records)
        const result = await (0, database_1.runAsync)(`DELETE FROM tasks WHERE id IN (${placeholders})`, taskIds);
        // Emit events
        for (const taskId of taskIds) {
            (0, eventBus_1.emitEvent)('task_deleted', { taskId });
        }
        return {
    ...
    try {
        for (const taskId of taskIds) {
            for (const tagId of tagIds) {
                try {
                    await (0, database_1.runAsync)('INSERT OR IGNORE INTO task_tags (task_id, tag_id) VALUES (?, ?)', [taskId, tagId]);
                    addedCount++;
                }
                catch (err) {
...
async function bulkRemoveTags(taskIds, tagIds) {
    ...
    try {
        const result = await (0, database_1.runAsync)(`DELETE FROM task_tags WHERE task_id IN (${taskPlaceholders}) AND tag_id IN (${tagPlaceholders})`, [...taskIds, ...tagIds]);
        return {
...
async function bulkDuplicateTasks(taskIds, userId = null) {
    ...
    try {
        ...
        const placeholders = taskIds.map(() => '?').join(',');
        const tasks = await (0, database_1.allAsync)(`SELECT * FROM tasks WHERE id IN (${placeholders})`, taskIds);
        if (tasks.length === 0) {
            return { created: [], errors: ['No tasks found'] };
        }
        for (const original of tasks) {
            // Create duplicate
            const result = await (0, database_1.runAsync)(`INSERT INTO tasks (
          ...
          execution_status, project_id, category
            ...
            const tags = await (0, database_1.allAsync)('SELECT tag_id FROM task_tags WHERE task_id = ?', [original.id]);
            for (const tag of tags) {
                await (0, database_1.runAsync)('INSERT INTO task_tags (task_id, tag_id) VALUES (?, ?)', [result.lastID, tag.tag_id]);
            }
            // Copy subtasks for this task
            const subtasks = await (0, database_1.allAsync)('SELECT * FROM subtasks WHERE task_id = ? ORDER BY position', [original.id]);
            for (const subtask of subtasks) {
                await (0, database_1.runAsync)('INSERT INTO subtasks (task_id, title, completed, position) VALUES (?, ?, ?, ?)', [result.lastID, subtask.title, 0, subtask.position]);
            }
        }
        return {
            created: newTaskIds,
