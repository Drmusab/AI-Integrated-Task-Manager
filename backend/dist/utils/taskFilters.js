...
async function searchTasks(options = {}) {
    ...
    ${paginationClause}
  `;
    const tasks = await (0, database_1.allAsync)(sql, params);
    // Load tags and subtasks for each task
    for (const task of tasks) {
        task.tags = await (0, database_1.allAsync)(`
      SELECT t.id as tag_id, t.name as tag_name, t.color as tag_color
      FROM tags t
      INNER JOIN task_tags tt ON t.id = tt.tag_id
      WHERE tt.task_id = ?
    `, [task.id]);
        task.subtasks = await (0, database_1.allAsync)(`
...
async function countTasks(filters = {}) {
    ...
    ${whereClause}
  `;
    const result = await (0, database_1.allAsync)(sql, params);
